...
for i in 1:length(mmm)
    if imag(mmm[i]) == 0 && real(mmm[i]) > i*treshold
...
print("Your results will be saved as: Normalized_reluctant_communities_$(a[k]) \n")
writedlm("../Resultados/Normalized_reluctant_communities_$(a[k])",comunidades)
